...
def lnprob_vertical(x, x_arr, err_x_arr, y_arr, err_y_arr):
    ...
    if np.log(sigma) < -1. or np.log(sigma)>2. or slope < -10 or slope > 10 or intercept < -10 or intercept > 10:
        return -1.e300
...
# set up the sampler
sampler = emcee.EnsembleSampler(Nens, ndims, lnprob_vertical, args=argslist)
...
figure = corner.corner(
    flat_samples,
    # title_fmt=".2E",
    levels=(1.-np.exp(-1), 1.-np.exp(-2.0)), 
...
for i in range(ndims):
    ...
    print(round(mcmc[1],3), round(q[0],4), round(-q[1],4))
    # display(Math(txt))
plt.show()