...
velocities=['values1',"values2","values3","values4","values5","values6",]
titles=['V_{f}','V_{2.2}','V_{2R_e}','V_{max}','W_{P20}/2','W_{M50}/2',]
index=-1
for v in velocities:
    index+=1
    ...
    figure.savefig('figures/corner_'+v+'.png',format='png', dpi=300)
    ...
    plt.plot(x_line, line[0]*x_line+line[1],'-',color='darkorange',linewidth=4,label='Best-fit line')
    ...
    Y1,Y1_e,X1,X1_e = np.loadtxt(v+'.txt', unpack=True)
